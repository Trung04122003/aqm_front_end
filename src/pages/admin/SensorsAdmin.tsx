...
export default function SensorsAdmin() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.get("/admin/sensors"); // adjust endpoint
      setList(res.data || []);
  ...
  const save = async () => {
    if (!editing) return;
    if (editing.id) await api.put(`/admin/sensors/${editing.id}`, editing);
    else await api.post(`/admin/sensors`, editing);
    setShow(false);
    load();
  };
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4>Sensors</h4>
        <Button onClick={openCreate}>Create Sensor</Button>
      </div>
      <Table striped hover>
      ...
      <Modal show={show} onHide={() => setShow(false)}>
        <Modal.Header closeButton><Modal.Title>{editing?.id ? "Edit Sensor" : "Create Sensor"}</Modal.Title></Modal.Header>
        <Modal.Body>
          <Form>
